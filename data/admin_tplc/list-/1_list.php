			...
			<tr>
				...
				<td>
					<select name="keytype">
						...
						<?php } ?>
						<?php if($m_rs[if_des]){?>
							<option value="note"<?php if($keytype == "note"){?> selected<?php } ?>>摘要</option>
						<?php } ?>
						<?php $_i=0;$m_key_list=(is_array($m_key_list))?$m_key_list:array();foreach($m_key_list AS  $key=>$value){$_i++; ?>
